 ...
 /**
  ...
  protected function AddAnAddress($kind, $address, $name = '') {
    if (!preg_match('/^(to|cc|bcc|Reply-To)$/', $kind)) {
      ...
      if ($this->exceptions) {
        throw new \Core\Mail\phpmailerException('Invalid recipient array: ' . $kind);
      }
	  if ($this->SMTPDebug) {
    ...
    if (!self::ValidateAddress($address)) {
      ...
      if ($this->exceptions) {
        throw new \Core\Mail\phpmailerException($this->Lang('invalid_address').': '.$address);
      }
	  if ($this->SMTPDebug) {
 ...
 */
  ...
  public function Send() {
    ...
    } catch (\Core\Mail\phpmailerException $e) {
  ...
  protected function PreSend() {
    try {
      ...
      if ((count($this->to) + count($this->cc) + count($this->bcc)) < 1) {
        throw new \Core\Mail\phpmailerException($this->Lang('provide_address'), self::STOP_CRITICAL);
      }
      ...
      if (empty($this->Body)) {
        throw new \Core\Mail\phpmailerException($this->Lang('empty_message'), self::STOP_CRITICAL);
      }
    ...
    if ($this->SingleTo === true) {
      foreach ($this->SingleToArray as $key => $val) {
        if(!@$mail = popen($sendmail, 'w')) {
          throw new \Core\Mail\phpmailerException($this->Lang('execute') . $this->Sendmail, self::STOP_CRITICAL);
        }
        fputs($mail, "To: " . $val . "\n");
        fputs($mail, $header);
        fputs($mail, $body);
  ...
  protected function MailSend($header, $body) {
    ...
    if(!$rt) {
      throw new \Core\Mail\phpmailerException($this->Lang('instantiate'), self::STOP_CRITICAL);
    }
    return true;
  }
  ...
  protected function SmtpSend($header, $body) {
    ...
    if(!$this->SmtpConnect()) {
      throw new \Core\Mail\phpmailerException($this->Lang('smtp_connect_failed'), self::STOP_CRITICAL);
    }
    $smtp_from = ($this->Sender == '') ? $this->From : $this->Sender;
	
    if(!$this->smtp->Mail($smtp_from)) {
      throw new \Core\Mail\phpmailerException($this->Lang('from_failed') . $smtp_from, self::STOP_CRITICAL);
    }
    ...
    if (count($bad_rcpt) > 0 ) { //Create error message for any bad addresses
      ...
      throw new \Core\Mail\phpmailerException($this->Lang('recipients_failed') . $badaddresses);
    }
    if(!$this->smtp->Data($header . $body)) {
      throw new \Core\Mail\phpmailerException($this->Lang('data_not_accepted'), self::STOP_CRITICAL);
    }
        ...
        if ($this->smtp->Connect(($ssl ? 'ssl://':'').$host, $port, $this->Timeout)) {
          ...
          if ($tls) {
            if (!$this->smtp->StartTLS()) {
              throw new \Core\Mail\phpmailerException($this->Lang('tls'));
            }
          ...
          if ($this->SMTPAuth) {
            if (!$this->smtp->Authenticate($this->Username, $this->Password)) {
              throw new \Core\Mail\phpmailerException($this->Lang('authenticate'));
            }
          }
        }
        $index++;
        if (!$connection) {
          throw new \Core\Mail\phpmailerException($this->Lang('connect_host'));
        }
      }
    } catch (\Core\Mail\phpmailerException $e) {
      $this->smtp->Reset();
  ...
  */
    ...
    if ($this->IsError()) {
      ...
      try {
        ...
        if (@openssl_pkcs7_sign($file, $signed, "file://".$this->sign_cert_file, array("file://".$this->sign_key_file, $this->sign_key_pass), NULL)) {
          ...
          @unlink($file);
          @unlink($signed);
          throw new \Core\Mail\phpmailerException($this->Lang("signing").openssl_error_string());
        }
      } catch (\Core\Mail\phpmailerException $e) {
        $body = '';
        if ($this->exceptions) {
  ...
  public function AddAttachment($path, $name = '', $encoding = 'base64', $type = 'application/octet-stream') {
    try {
      if ( !@is_file($path) ) {
        throw new \Core\Mail\phpmailerException($this->Lang('file_access') . $path, self::STOP_CONTINUE);
      }
  ...
  */
  ...
  protected function EncodeFile($path, $encoding = 'base64') {
    try {
      if (!is_readable($path)) {
        throw new \Core\Mail\phpmailerException($this->Lang('file_open') . $path, self::STOP_CONTINUE);
      }
  ...
  public function set($name, $value = '') {
    try {
      if (isset($this->$name) ) {
        ...
        throw new \Core\Mail\phpmailerException($this->Lang('variable_set') . $name, self::STOP_CRITICAL);
      }
    } catch (Exception $e) {
      $this->SetError($e->getMessage());
      if ($e->getCode() == self::STOP_CRITICAL) {
