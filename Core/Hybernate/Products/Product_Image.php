...
class Product_Image extends \Core\Interfaces\HybernateInterface 
{
	...
	public function getImagePath($intImagePosition = null) 
	{
		...
		$strImagePath = \Core\Hybernate\Products\Product_Image::getDefaultImage($intImagePosition);
		...
		if ($blnReturn) {
			foreach($arrImagePositions as $intIndex => $arrRow) {
				...
				if ($blnReturn) {
					...
					// Changed from RESIZE_TYPE_NOT_BOX tp RESIZE_TYPE_BOX
					$objNewItemImagePosition->setQuality(100);
					$objNewItemImagePosition->setPreserveAspectRatio(true);
					$blnReturn = $objNewItemImagePosition->resize(
						(int) $arrRow['width'], 
						(int) $arrRow['height'],
						\Core\Io\Image\Image::RESIZE_TYPE_BOX
					);
				}
			}
		}
	...
	/**
	 * This method returns a default image
	 *
	 * @return string
	 */
	 public static final function getDefaultImage($intImagePosition = 0)
	 {
