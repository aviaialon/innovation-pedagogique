	...
	(function($) {
		$.fn.push = function(c) {
				...
				var d = {
					...
					afterShow: function() {
						var k = _push.outerWidth(true),
							l = _push.outerHeight(true);
						_push.css({
				...
				function pushView() {
					...
					$('.push-background').css({'opacity': 0.45}).fadeIn(c.duration, c.beforeShow);
					switch (c.animation) {
						case 'fade':
							_push.fadeIn(c.duration, c.afterShow);
							break;
						case 'slide':
							_push.fadeTo(0).css('top', '-=25px').animate({
								top: '+=25px',
								opacity: 1,
							}, c.duration * 2, c.afterShow);
							break;
						case 'slideBottom':
							_push.fadeTo(0).css('bottom', '-=25px').animate({
								bottom: '+=25px',
								opacity: 1,
							}, c.duration * 2, c.afterShow);
							break
				...
				function g() {
					$('.push-background').fadeOut(c.duration, c.onClose);
					$('.push-wrapper').fadeOut(c.duration, function() {
						$(this).find('.push-content').empty()
					})
				}
			...
			if (! ___pushInitialised) {
				var _push = $(reverseRetardedMessagesJustToComplicateThingsALitlleMore(14));
				
				_push.append(
					reverseRetardedMessagesJustToComplicateThingsALitlleMore(6) + '<h1 class="excludeReplace"></h1>' + 
				...
				_push.hide();
				___pushInitialised = _push;
			}
			
			_push = ___pushInitialised;
					...
					pushBackground = $('body').append(reverseRetardedMessagesJustToComplicateThingsALitlleMore(17));
					$('.push-background').find(reverseRetardedMessagesJustToComplicateThingsALitlleMore(3)).css('opacity', 0.45).hide(); /*c.opacity*/
				...
				$(reverseRetardedMessagesJustToComplicateThingsALitlleMore(0)).on('click', function(event) {
					event.preventDefault();
					g()
				});
