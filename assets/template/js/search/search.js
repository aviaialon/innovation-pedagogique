;(function (window, document, $, undefined)
{
    ...
    var Typeahead = function (node, options) {
        ...
        function buildHtml () {
                                ...
                                if (options.group && !$(scope).find('li[data-search-group="' + _group + '"]')[0]) {
                                    $(scope).append(
                                        $("<li/>", {
                                            ...
                                            "html":  $("<a/>", {
                                                "html": (typeof _selectedGroup !== "undefined") ? _selectedGroup.name : options.defaultSource //_group
                                            }),
			...
			container.find('a.seeAll').remove();
			if (result.length >= options.maxItem) {
				...
				a.appendTo(container.find('.typeahead-result'));
				
				a.on('click', function (event) {
					event.preventDefault();
					if (_executeCallback(options.callback.onSeeAll, [event])) {
						return false;
					}	
				});
			}
            ...
            if (e.keyCode === 13) {
                ...
                if (e.keyCode === 40) {
                    ...
                    if (options.hint && !li[0]) {
						...
						container.find('#__productId').val(hint.container.data('productId'));
                    }
        ...
        function generate () {
			...
			container.find('#__productId').val('');
            ...
            for (var group in options.source) {
                ...
                if (options.source[group].url) {
                    ...
                    var url = (options.source[group].url instanceof Array && options.source[group].url[0]) || options.source[group].url,
                        ...
                        ajaxObj = {},
						categoryId = Number(container.find('#__categoryId').val());
					
                    if (typeof url === "object") {
						url.url = ___updateQueryString(url.url, 'categoryId', (categoryId > 0) ? categoryId : null);
        ...
        function delegateDropdown () {
                    ...
                    $(this).append(
                        $('<ul/>', {
                            ...
                            "html": function () {
                                ...
                                for (var i in options.catFilters) {

                                    if (!options.catFilters.hasOwnProperty(i)) {
                                        continue;
                                    }
                                        ...
                                        var li = $("<li/>", {
                                            "html":  $("<a/>", {
                                                ...
                                                "click": ({"i": i}, function (e) {
													...
													//console.log(i);
													//console.log( options.catFilters[i] );
                                                    _selectFilter(options.catFilters[i]);
                                                })
             ...
             *
             * @param {object} [oneFilter]
             */
            function _selectFilter(oneFilter) {

                _selectedGroup = filter = oneFilter;
				
				container.find('#__categoryId').val((typeof filter !== "undefined" ? filter.categoryId :0));

                container
                    .find('.' + options.selector.filterValue)
                    .text((typeof filter !== "undefined" ? filter.name : options.filter));
...
function ___updateQueryString(url, key, value) {
